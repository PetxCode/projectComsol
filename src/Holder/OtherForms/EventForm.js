...
const EventForm = () => {
	...
	const onSubmit = handleSubmit(async (data) => {
		console.log(data);
		const url = `https://project-comsol.herokuapp.com/api/event/${id}/created`;
		...
		<Container>
			<Wrapper>
				<Content>
					<Holder>
						<HolderCard>
							<Welcome>Create an Event Captured Moment</Welcome>
							<SubTitle>
								Glad you are here... Let's help get seen, start by sharing your
								most recent Event done
							</SubTitle>
							<Form onSubmit={onSubmit} type="multipart/form-data">
								<ImageHolder>
									...
									<ImageLabel htmlFor="img">
										Choose an event cover image
									</ImageLabel>
								</ImageHolder>

								<Label>Event Capture</Label>
								<LabelError>{errors.title && errors?.title.message}</LabelError>
								<Input placeholder="Event Capture" {...register("title")} />
								...
								<Button type="submit">Upload this Event </Button>
							</Form>
						</HolderCard>
					</Holder>
				</Content>
